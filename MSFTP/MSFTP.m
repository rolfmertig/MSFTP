...
ReinstallJava[JVMArguments -> "-Xmx" <> If[ NumberQ[$SystemMemory],
                                            ToString@Round[0.3 $SystemMemory/1024^2],
                                            "512"
                                        ] <> "m"];
...
PassDecode[li : {__Integer}] :=
    ReleaseHold[ImportString[FromCharacterCode[li], "Package"]];
        ...
        Catch[Module[ {channel, conf, jsch, util, username, host, port, pwd, session},
                  ...
                  *)
                  ...
                  If[ session[isAuthenticated[]] =!= True,
                      Message[openchan::authenticated, username, host];
                      Throw[$Failed]
                  ];
                  channel = session[openChannel[prot]];
                  channel[connect[]];
                  If[ channel[isConnected[]] === False,
                      Message[openchan::connected, username, host];
                      Throw[$Failed]
                  ];
                  channel
              ]]
...
MSFTPGet[remotefileordir_String, localdir_String:Directory[], opts:OptionsPattern[] ] :=
    Block[ {getAttrs, getSize, toArray, get, disconnect, getFilename, ls, i,stat, isDir, cd, pwd},
        Catch @ Module[ {channel, filesize, lsrem, localfilenames, print, localdirs, remotedirs, remotefilesanddirs, remotefiles, directoryQ, rdir},
                    ...
                    If[channel === $Failed, Throw[$Failed]];
                    ...
                    If[ lsrem === $Failed,
                        Message[MSFTP::notfound, remotefileordir];
                        Throw[$Failed]
                    ];
...
putfile[chan_, file_String?FileExistsQ] :=
    Block[ {put},
        Module[ {filesize, flocal},
...
chdir[chan_, r_String /; r =!= "."] :=
    Block[ {lsrem, ls, mkdir, cd},
...
putdir[chan_, dir_String?DirectoryQ, opts___?OptionQ] :=
    Block[ {cd, put, pwd},
    ...
    Block[ {cd, getAttrs, getSize, toArray, get, disconnect, getFilename, mkdir, setProperty, ls, put},
        Catch@Module[ {channel, dir = Directory[]},
                  ...
                  If[ FileType[local] === File,
                      putfile[channel, local]
                  ];
                  If[ FileType[local] === Directory,
                      putdir[channel, local, opts]
                  ];
                  channel@disconnect[];
                  SetDirectory[dir];
                  local
              (* return the directory being uploaded *)
                ]
    ]
